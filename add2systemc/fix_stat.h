...
// this signed structure also collects statistics, and can be connected in a linked list
...
class fix_stat:
    ...
    fix_stat( const sc_fxtype_params& typeinfo_, fix_stat* next_ = NULL, const char* name_ = "")
	: sc_fix(typeinfo_) {
		count=0;
		smin= DBL_MAX;
		smax= -DBL_MAX;
    ...
    fix_stat(fix_stat* next_ = NULL, const char* name_ = "")
	: sc_fix() {
		count=0;
		smin= DBL_MAX;
		smax= -DBL_MAX;
	...
	double sqnr() {
		...
		else {result = 10 * log10 ((sigmean()*sigmean()+sigdev()* sigdev())/(errormean()* errormean()+errordev()* errordev()));}
		return(result);
		}
	double minsqnr() {return(sqnrmin);}
	fix_stat* next_el() {return (next);}
	char* name() { return (my_name);}

	// write operations
	void set_next(fix_stat* next_el) {next = next_el;}
